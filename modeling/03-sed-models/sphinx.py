...
def sphinx_model_spectra(bin_wl, path=None):
    ...
    return (unique_temps.astype(jnp.float32),
            unique_loggs.astype(jnp.float32),
            unique_metals.astype(jnp.float32),
            unique_ctos.astype(jnp.float32),
            bin_wl.astype(jnp.float32),
            sphinx_grid.astype(jnp.float32))

# def get_interp_stellar_spectrum(wl, bin_edge_wl, path=None):
    
#     temp_grid, metal_grid, cto_grid, sphinx_grid = sphinx_model_spectra(bin_edge_wl, path)
